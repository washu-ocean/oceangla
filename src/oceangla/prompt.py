...
def prompt_space(db_path: str) -> str:
    ...
    idx_to_space = {str(i + 1): space for (i, space) in zip(range(len(unique_spaces)), unique_spaces)}

    def _get_space_input():
        for idx, space in unique_spaces.items:
            print(f"{idx}\t{space}")
        return input("Please enter the number associated with the template space these analyses should run in: ")
    chosen_opt = None
...
def prompt_task(db_path: str) -> str:
    ...
    idx_to_task = {str(i + 1): task for (i, task) in zip(range(len(unique_tasks)), unique_tasks)}
