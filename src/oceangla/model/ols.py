...
class OLSModel:
    def __init__(self,
        ...
        if self.image_type == "NIFTI":
            ...
            for k, v in tflow.get_metadata(self.space)["res"].items():
                if np.allclose(self.header.get_zooms()[:3], v["zooms"]):
                    self.volume_mask = nib.load(tflow.get(
                        self.space,
        ...
        self.l_neigh = None if self.no_surfs else build_adjacency_from_faces(self.l_numverts, self.l_faces)
        self.r_neigh = None if self.no_surfs else build_adjacency_from_faces(self.r_numverts, self.r_faces)
    ...
    def _fit(self, permuted_design_matrix=None):
        ...
        spatial_slices, no_of_spatial_slices = get_spatial_slices(self.activation, self.image_type, self.volume_mask)
        pvals, tstats, betas, ses = (np.full(img_shape, np.nan, dtype=np.float32),) * 4
        with progressbar.ProgressBar(max_value=no_of_spatial_slices, redirect_stdout=True) as pbar:
    ...
    def __add_cifti_surf_cluster_sizes(self, pvals: np.ndarray):
        ...
        for alpha in self.alphas:
            self.__biggest_l_surf_cluster_sizes[alpha].extend(get_biggest_clusters_from_pmap(l_pvals, alpha, self.l_neigh, self.l_area))
            self.__biggest_r_surf_cluster_sizes[alpha].extend(get_biggest_clusters_from_pmap(r_pvals, alpha, self.r_neigh, self.r_area))
        ...
        for alpha_idx, alpha in enumerate(self.alphas):
            for value_idx in range(len(self.value_names)):
                ...
                for cluster in get_cluster_index_groups(l_mask, self.l_neigh):
                    ...
                    sizes_larger_than_this_cluster = np.float32(np.sum(self.__biggest_l_surf_cluster_sizes[alpha]) >= cluster_size)
                ...
                for cluster in get_cluster_index_groups(r_mask, self.r_neigh):
                    ...
                    sizes_larger_than_this_cluster = np.float32(np.sum(self.__biggest_r_surf_cluster_sizes[alpha]) >= cluster_size)
    ...
    def _save_cifti(self):
        for datatype, data in (
            ...
            nib.save(img, p := self.model_outdir / f"{sanitize_filename(self.model_desc)}_{datatype}.dscalar.nii")
            logger.info(f"Saved {p!s}")
            del img
    ...
    def _save_nifti(self):
        for datatype, data in (
            ...
            if data is None:
                continue
            for idx, value_name in enumerate(self.value_names):  # Have to save a different image per-beta bcuz NIFTI doesn't have volume labelling :-(
                img = nib.Nifti1Image(data[..., idx], self.affine, header=self.header)
                nib.save(img, p := self.model_outdir / f"{sanitize_filename(self.model_desc)}_beta-{value_name}_{datatype}.nii.gz")
                logger.info(f"Saved {p!s}")
                del img
